#first in the clean data directory
ls | sed /name/d > name
...
#For the clean data got from the company, it has no adapters need to be trimmed and the data quality is very good, so we use it directly for the following analysis
#mapping
...
#use featurecounts got the raw read count of genes and transposons
