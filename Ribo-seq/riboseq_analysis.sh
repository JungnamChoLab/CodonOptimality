...
# RNA-seq analysis
# The raw data were firstly processed through in-house perl scripts to remove reads containing adapter, ploy-N and low quality by the company. Clean reads were used for the following analysis
for i in `cat name`;do echo $i;hisat2 -p 15 --dta --rg-id $i --rg SM:$i --rna-strandness RF --fr -x ~/genome/tair10seq -1 ../clean_data/${i}_1.clean.fq.gz -2 ../clean_data/${i}_2.clean.fq.gz -S ${i}.sam >> mapping-record 2>&1;
samtools view -bSF 4 ${i}.sam > ${i}.us.bam;samtools sort -@ 15 -o ${i}.bam ${i}.us.bam;samtools index ${i}.bam;
done
